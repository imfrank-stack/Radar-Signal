...
RadChar is a PyTorch-based radar signal classification model using **CNN + Mamba3 + ProbSparse Self-Attention** architecture for IQ data sequences (length 1024). Supports 12 signal classes (configurable via `radchar_classes.json`).
...
# Time-frequency mode: STFT or GLCT
...
--batch_size 16       # Default 16 (1024 seq len)
...
--timefreq_mode       # fft (default), stft, or glct
--compile             # Enable torch.compile
```

## Architecture

**Input Pipeline**: I (B, 1024) + Q (B, 1024) → ComplexInputAugment (CFO/STO) → IQInputProcessor → (B, 3, 1024) I/Q/时频特征 (FFT/STFT/GLCT)
...
- IQ shape: (num_samples, 1024), complex values
